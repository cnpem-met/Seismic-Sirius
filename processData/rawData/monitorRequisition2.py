...
seismicDB = mysql.connector.connect(
  host="localhost",
  user="reftek",
  password="reftek",
  database="seismicsirius"
)
mycursor = seismicDB.cursor()
...
class monitorMseedData(threading.Thread):
    ...
    def atualizaArquivo(self, dados, canal):
        sql = "INSERT INTO returndata (id, retorno) VALUES (%s, %s)"
        val = (name+"_channel"+str(canal), dados)
        mycursor.execute(sql, val)
        seismicDB.commit()
        print(mycursor.rowcount, "gravação inserida canal %d" % canal)
            ...
            if novoArquivo:
                ...
                for arquivo in novoArquivo:
                    if ".msd" in arquivo:
                        ...
                        self.atualizaArquivo(novoDado, canal)
